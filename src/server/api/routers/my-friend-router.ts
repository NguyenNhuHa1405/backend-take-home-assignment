    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.connection().execute(async (conn) =>
        ...
        conn
          ...
          .innerJoin('friendships as f', 'f.friendUserId', 'friends.id')
          .innerJoin(
          ...
          .leftJoin(
            () => mutualFriendCount(conn).as('mfc'),
            (join) =>
              join
                .onRef('mfc.userId', '=', 'f.userId')
                .onRef('mfc.friendUserId', '=', 'f.friendUserId')
          )
          .where('f.userId', '=', ctx.session.userId)
          .where('f.friendUserId', '=', input.friendUserId)
          .where('f.status', '=', FriendshipStatusSchema.Values['accepted'])
          .select([
            'friends.id',
            'friends.fullName',
            'friends.phoneNumber',
...
const mutualFriendCount = (db: Database) => {
  return db
    ...
    .innerJoin('friendships as f2', 'f2.friendUserId', 'f1.userId')
    .innerJoin('friendships as f3', 'f3.friendUserId', 'f1.friendUserId')
    .whereRef('f2.userId', '=', 'f3.userId')
    .where('f1.status', '=', FriendshipStatusSchema.Enum.accepted)
    .where('f2.status', '=', FriendshipStatusSchema.Enum.accepted)
    .where('f3.status', '=', FriendshipStatusSchema.Enum.accepted)
    .select((eb) => [
      'f1.userId',
      'f1.friendUserId',
      eb.fn.countAll().as('mutualFriendCount'),
    ])
    .groupBy(['f1.userId', 'f1.friendUserId'])
}
